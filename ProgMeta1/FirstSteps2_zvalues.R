...
## transform all numeric variables into z-values

fblist_num <- lapply(fblist_num, function(df) {
  df_select <- df %>% select(-contains("CODE")) # select columns except for "CODE"
  df_select_scaled <- scale(df_select) # scale the selected columns
  df_select_scaled_df <- as.data.frame(df_select_scaled) # convert to data frame
  df_code <- df %>% select(contains("CODE")) # select only the "CODE" column
  result <- cbind(df_select_scaled_df, df_code) # combine the scaled data and "CODE" column
  return(result)
})
...
####################################################################
############### Stiftungsbericht Daten Aggregation #################
####################################################################


### see syntax FirstSteps2_Aggregate_SK.R
