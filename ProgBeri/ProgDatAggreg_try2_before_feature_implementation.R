...
for (k in 1:length(fblist2_num)) {
  ...
  for (i in seq_along(scalevar_dfs)) {
    ...
    if (ncol(scalevar_dfs[[i]]) > 2) {
      ...
      #### ... this needs to be extended with a list of scale types and calculation rules
...
#### Group the list into Programm-ID_ID groups
library(stringr)
combined_dfs_list3 <- split(combined_dfs_list2, sub("^(.*?)_(.*?)_(.*)$", "\\1_\\2", sapply(names(combined_dfs_list2), function(x) unlist(strsplit(x, split = ".", fixed = TRUE))[1])))
...
for (i in seq_along(combined_dfs_list3)) {
  if (length(combined_dfs_list3[[i]]) >= 2 && "CODE" %in% names(combined_dfs_list3[[i]][[1]]) && "CODE" %in% names(combined_dfs_list3[[i]][[2]])) {
    # join dfs
...
fblist_num4 <- combined_dfs_list3
