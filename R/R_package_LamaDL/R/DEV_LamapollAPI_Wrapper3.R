##### API Wrapper for Lamapoll Data Download

#' Lamapoll Data Download
#'
#' This function Let's you download Data from Lamapoll through their API. It's specified to the DKJS API key. Do not share this package openly. Upon using, the package will create a folder in the path entered in the function (second parameter: "xdir"). The data will be stored in individual excel files for each questionnaire in this folder.  
...
#' @return A number of excel files.
#' @examples 
#' backup.lamapoll <- function("online", "C:/Users/Alexander Wedel/OneDrive - Deutsche Kinder- und Jugendstiftung/Auswertung/Daten/Backup/");
#' @import openxlsx
#' @import httr
#' @import fs
#' @import jsonlite
...
backup.lamapoll <- function(xstatus, xdir) {
  ...
  get_lamapoll_list <-function(email, fb_status, apikey){
    ...
    user_token <- openssl::sha256(paste0(auth_token, "wGl5v4fz"))
    ...
    api_token <- openssl::sha256(paste0(requestToken, "wGl5v4fz"))
    # get data
  ...
  if (xstatus == "online" || xstatus == "offline") {
    ...
    fb_names <- as.data.frame(get_lamapoll_list("lamapoll@dkjs.de", xstatus, "wGl5v4fz"))
    names(fb_names)[1] <- "fb_names"
    list_status <- as.list(fb_names$fb_names)
    
    p_list_df_status <- lapply(list_status, function (x) get_lamapoll_data("lamapoll@dkjs.de", x, "wGl5v4fz"))
    ...
    dir_create(dir_status)
  
    file_ending_status <- paste0("_", xstatus, file_sysdate, ".xlsx")
    lapply(p_list_df2_status, function (x) write.xlsx(x, paste0(dir_status, "/", names(p_list_df2_status[]), file_ending_status)))
  }
  
  else if (xstatus == "selection") {
    
    programmliste <- read.xlsx("C:/Users/Alexander Wedel/OneDrive - Deutsche Kinder- und Jugendstiftung/Auswertung/Programmliste.xlsx", startRow = 1)
    p_list <- as.list(programmliste$FB_name)
    
    p_list_df <- lapply(p_list, function (x) get_lamapoll_data("lamapoll@dkjs.de", x, "wGl5v4fz"))
    ...
    dir_create(dir_selection)
    file_ending <- paste0("_selection", file_sysdate, ".xlsx")
    lapply(p_list_df2, function (x) write.xlsx(x, paste0(dir_selection, "/", names(p_list_df2[]), file_ending)))
  }
...
backup.lamapoll("inwork", "C:/Users/Alexander Wedel/OneDrive - Deutsche Kinder- und Jugendstiftung/Auswertung/Daten/Backup/")
